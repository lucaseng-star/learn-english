...
notify = (f'<div style="width:390px;height:260px;background:{CANVAS};display:flex;flex-direction:column;gap:10px;padding:16px;box-sizing:border-box;font-size:15px;line-height:1.45">'
    ...
    f'<div style="font-size:13.5px;line-height:1.4;color:{INK2};margin-top:2px">🍷 Emily 线 · 问：刚毕业，中途找到全职工作可以转周末班吗？学费怎么算？</div>'
    f'<div style="font-size:12px;color:{MUTED};margin-top:4px">点开 = 认领 + 停 bot + 键盘弹出。别人先点了会显示「Boon 已接」。</div></div></div>'
...
main_inner = (header2('Sharon', '🍷', 'Emily 线', st('已认领 · 你','green') + st('Bot 已停','green') + st('对客户显示 Emily','gray'))
...
reply_inner = (header2('Aina', '☕', 'Coco 线', st('已认领 · 你','green') + st('Bot 已停','green') + st('自动翻译 开','blue'))
    + ctxline(['1-Day Junior Barista', 'BM', '好奇'], '⚠ BM 渠道不放酒精')
    + stream2([daysep('今天'),
    ...
    + f'<div style="border-top:1px solid {LINE};background:{SURF}">'
    + drafts_row([draft_card('有周末场 · 下一场 20/9 · 报价', 'Ada! Sesi hujung minggu seterusnya 20 Sept (Sabtu), 10 pagi–2 petang. Harga RM ___ termasuk bahan &amp; sijil kehadiran. Nak saya simpan tempat?', '目录 ✓', 286, '中文：有！下一场周末 9/20 周六 10–2 点，价含材料和出席证明，要留位吗？'),
...
gate_inner = (header2('Sharon', '🍷', 'Emily 线', st('未认领','yellow') + st('Bot 在回','yellow') + st('对客户显示 Emily','gray'))
    + ctxline(['WNSM 调酒半工读', '中文', '热'])
    + stream2([daysep('今天'), bubble_in('你好 我看到调酒半工读的广告', '11:02'),
               bubble_out('哈喽 Sharon，我是 Emily 🍷 调酒半工读是 9 个月 36 堂，一周上 1 天课，其余时间在合作酒吧带薪实习。你现在是在工作，还是刚毕业？', '11:02', 'Emily · Bot'),
               bubble_in(SHARON_Q, '11:04'), sos_marker('11:04')])
    + f'<div style="background:{WARM};padding:0 0 10px">{task_card(SHARON_Q, "等了 14 分 · 承诺 5-10")}</div>'
    + composer2('', '以 Emily 的身份回复…', disabled=True))
(OUT/'Gate.dc.html').write_text(phone(gate_inner))

# ══════════ 4. Sent —— 发完一行提示，不逼决定 ══════════
sent_inner = (header2('Sharon', '🍷', 'Emily 线', st('已认领 · 你','green') + st('Bot 已停','green') + st('对客户显示 Emily','gray'))
    + ctxline(['WNSM 调酒半工读', '中文', '热'])
    + stream2([daysep('今天'), bubble_in('你好 我看到调酒半工读的广告', '11:02'),
               bubble_out('哈喽 Sharon，我是 Emily 🍷 调酒半工读是 9 个月 36 堂，一周上 1 天课，其余时间在合作酒吧带薪实习。你现在是在工作，还是刚毕业？', '11:02', 'Emily · Bot'),
               bubble_in(SHARON_Q, '11:04'), sos_marker('11:04'),
    ...
    f'<div style="flex:1;min-width:0"><div class="num" style="font-size:11.5px;font-weight:600;color:rgba(255,255,255,.7)">清队列 1 / 3 · 下一条</div><div style="font-size:14px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">Wei Jie · ☕ Cindy 线 · <span class="num" style="color:#f0c66a">等 9 分</span></div></div>'
...
quick_drawer = (f'<div style="position:absolute;left:0;right:0;bottom:0;height:640px;display:flex;flex-direction:column;border-radius:22px 22px 0 0;background:{SURF};box-shadow:0 -2px 24px rgba(16,24,40,.16)">'
    ...
    f'<div style="display:flex;align-items:center;gap:8px;height:42px;padding:0 12px;border-radius:999px;background:{SURF2};box-shadow:0 0 0 2px rgba(46,192,106,.45);font-size:15px;color:{INK}">{ic("search",18,"color:#7b838e;")}价格<span style="display:inline-block;width:1.5px;height:17px;background:{INK};vertical-align:-3px;margin-left:1px"></span><span style="margin-left:auto;font-size:12px;color:{TIME}">4 条</span></div>'
...
quick_inner = (header2('Sharon', '🍷', 'Emily 线', st('已认领 · 你','green') + st('Bot 已停','green'))
    + ctxline(['WNSM 调酒半工读', '中文', '热'])
...
queue_inner = (f'<div style="display:flex;align-items:center;gap:8px;padding:16px 8px 6px 16px"><h1 style="margin:0;font-size:21px;font-weight:700;letter-spacing:-.015em">待回</h1><span style="font-size:12.5px;color:{MUTED};margin-top:6px">主动清的时候才来这里</span><span style="flex:1"></span>{iconbtn("search")}{iconbtn("gear")}</div>'
    ...
    + dock(primary('开始清队列', '· 4 条（跳过别人在回的）')))
(OUT/'Queue.dc.html').write_text(phone(queue_inner))

# ══════════ 7. Failed —— 送不出去，大声且带下一步（Nurul · IG · Coco 线）══════════
fail_inner = (header2('Nurul', '☕', 'Coco 线 · IG', st('已认领 · 你','green') + st('Bot 已停','green') + st('自动翻译 开','blue'))
    + ctxline(['BWC Bakery Weekend', 'BM', '考虑中'], '⚠ IG 24 小时窗口已关')
    + stream2([daysep('昨天'),
               bubble_in_tr('my number 012-345 6789, boleh call petang. Nak tanya pasal kelas roti weekend tu', '昨天 15:10', '我的号码 012-345 6789，下午可以打。想问周末面包课的事'),
               ...
               f'<div style="margin-top:3px;font-size:12.5px;line-height:1.4;color:{INK2}">客户上次发言 昨天 15:10，超过 24 小时 Instagram 不让主动发。她留了 WhatsApp 号码，下一步：</div>'
...
v2_boards = [
    {"file":"Notify.dc.html","x":x(0),"y":0,"w":W,"h":260,"title":"0 · 通知 → 直达"},
    {"file":"Main.dc.html",  "x":x(1),"y":0,"w":W,"h":H,"title":"1 · 会话（从通知进来）"},
    {"file":"Reply.dc.html", "x":x(2),"y":0,"w":W,"h":H,"title":"2 · 翻译内建 + 草稿写答案"},
    {"file":"Gate.dc.html",  "x":x(3),"y":0,"w":W,"h":H,"title":"3 · 从队列进来 · 先停 bot"},
    {"file":"Sent.dc.html",  "x":x(0),"y":1020,"w":W,"h":H,"title":"4 · 发完一行提示"},
    {"file":"Quick.dc.html", "x":x(1),"y":1020,"w":W,"h":H,"title":"5 · 话术先搜索 · 插入前预览"},
    {"file":"Queue.dc.html", "x":x(2),"y":1020,"w":W,"h":H,"title":"6 · 队列 · 谁在回"},
    {"file":"Failed.dc.html","x":x(3),"y":1020,"w":W,"h":H,"title":"7 · 送不出去 → 下一步"},
]
v1_boards = []
...
v2_notes = [
    ...
    {"id":"v2-n3","x":x(3),"y":900,"w":W,"page":"v2","text":"只有从队列主动点进来才会看到这一屏。「停 bot 并回复」一下，就变成画板 1 的状态。"},
]
canvas = {
